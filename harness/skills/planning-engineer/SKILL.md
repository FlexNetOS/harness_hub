...
description: >-
  ...
  ASCII architecture diagrams, code-quality/speed/accuracy upgrades, and tool-evaluation. ALWAYS use
...
quality/speed/accuracy upgrade roadmap, and a tool-evaluation. The defining property (inherited from
...
plan/graph artifacts under `.handoff/loop/plan/` and (architect only) the docs/ROADMAP+ADR promotion.
...
| 2 Analyze | Sub-agent, **parallel** | one analyst per dimension → cited gaps + upgrades |
...
| `plan-test-strategist` | the always-on **`test-coverage`** dimension: map existing tests (by call-graph reachability) + design the suite that closes the gaps (unit/integration/e2e/golden/property/fail-closed) + the Feature-Forge test-build handoff | **testing** | specialist |
...
Skills used: `plan-cartography`, `plan-trend-research`, `plan-test-strategy`, `plan-synthesis`, the
reused `code-research-verify` (the verifier's refute discipline), `session-relay-wrap-up`,
...
- `plan-cartographer` → `.handoff/loop/plan/graph/<T>.{symbols,callgraph,metrics}.json` +
  ...
  *update*) + `reports/codemap-<T>.md`; seeds `dimensions.md` with the dimensions this target needs
  ...
  accuracy, tooling, …). Built **only** from `git-kb code` JSON (no C dep, no graph DB).
...
For each `- [ ]` dimension, spawn `plan-analyst` (parallel) → `.handoff/loop/plan/findings/<dim>.md`:
falsifiable **CLAIM** rows (each citing `file:line` / symbol / call-path / test) + named **gaps** +
**UPGRADE** rows each tagged `axis: quality|speed|accuracy` with rationale, evidence, blast-radius
...
dedicated **tool-evaluation** section (tools/CLIs/MCPs/crates the target uses, their 90-day currency /
...
architect emits an ASCII service diagram + a quality/speed/accuracy roadmap + a tool-eval (prost
