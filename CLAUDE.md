...
| 2026-06-13 | Continuous-autonomous-cadence rule in the standard + `LESSONS.md` row | `docs/packaged-harness-standard.md`, `harness/LESSONS.md` | Owner via `/harness:harness-evolution`: a loop must not stop-and-ask between items; run plan→implement→test→next to a ~50% context budget; only genuine walls halt. Per-orchestrator budget mechanics proposed (`.handoff/loop/proposed-upgrades.md`). |
